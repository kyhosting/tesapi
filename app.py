...
def _scrape_jwt_and_user(scraper, page_url: str) -> tuple:
    """
    Ambil JWT token + user hash untuk Socket.IO iVAS /livesms namespace.

    Dari source code iVAS asli (my_sms JS):
        window.liveSMSSocket = io('https://ivasms.com:2087/livesms', {
            query: {
                token: 'eyJ...',          ← JWT token
                user:  "e76c590db0526e1f8892645bcae9aa5e"  ← user hash (MD5 hex)
            },
        });

    PENTING — dari hasil debug:
      - XSRF-TOKEN cookie = "eyJpdiI6..." → INI JWT-nya! Bukan laravel_session.
      - user hash = hex 32 chars, sudah berhasil diambil dari HTML.
      - iVAS pakai Rocket Loader → JWT tidak ada di raw HTML sebagai eyJ.xxx.xxx
        tapi ada di cookie XSRF-TOKEN dalam format eyJpdi... (base64url encoded JSON)

    Return: (jwt_token: str|None, user_hash: str|None)
    """
    ...
    try:
        ...
        # ── Cek XSRF-TOKEN dulu — ini JWT-nya di iVAS! ──
        # Dari debug: XSRF-TOKEN = eyJpdiI6IkX... (ada di semua akun)
        # iVAS encrypt JWT pakai Laravel encryption → hasilnya eyJpdi...
        # Ini yang dikirim sebagai query.token ke Socket.IO
        xsrf = scraper.cookies.get("XSRF-TOKEN", "")
        if xsrf and xsrf.startswith("eyJ"):
            # URL-decode dulu (Laravel kadang encode + sebagai %2B)
            try:
            ...
            logger.info(f"[WS-JWT] ✅ JWT dari XSRF-TOKEN cookie: {jwt_tok[:35]}...")

        # ── Fallback: cari eyJ.xxx.xxx di HTML (standard JWT format) ──
        if not jwt_tok:
            all_jwts = re.findall(
            ...
            if all_jwts:
                jwt_tok = all_jwts[0]
                logger.info(f"[WS-JWT] ✅ JWT dari HTML eyJ.x.x: {jwt_tok[:30]}...")

        # ── Ekstrak user hash dari HTML ──
        # Pattern: user:"e76c590db0526e1f8892645bcae9aa5e"
        user_m = re.search(r"""[,{]\s*user\s*:\s*["']([a-f0-9]{32})["']""", html)
        if user_m:
            user_hash = user_m.group(1)
            logger.info(f"[WS-JWT] ✅ user hash dari HTML: {user_hash}")

        # Pattern: query: { token: ..., user: ... }
        if not user_hash:
            user_m2 = re.search(
                r'query\s*:\s*\{[^}]*user\s*:\s*["\']([a-f0-9]{24,})["\']',
                html
            )
            if user_m2:
                user_hash = user_m2.group(1)
                logger.info(f"[WS-JWT] ✅ user hash dari query object: {user_hash}")

        # ── Coba polling handshake kalau JWT masih kosong ──
        if not jwt_tok:
            try:
                ...
                poll_body = pr.text
                logger.info(f"[WS-JWT] Polling HTTP {pr.status_code}: {poll_body[:100]}")
        ...
        # ── user hash fallback: ambil dari XSRF-TOKEN jika belum ada ──
        # (XSRF-TOKEN terlalu panjang untuk user hash, pakai MD5-like bagian)
        if not user_hash:
            if xsrf:
                # Ambil 32 chars alphanum dari XSRF sebagai identifier
                clean_xsrf = re.sub(r'[^a-f0-9]', '', xsrf.lower())
                if len(clean_xsrf) >= 32:
                    user_hash = clean_xsrf[:32]
                    logger.info(f"[WS-JWT] user hash fallback dari XSRF: {user_hash}")

        status = "✅" if jwt_tok else "❌"
        logger.info(
            f"[WS-JWT] {status} Result: "
            f"token={'ada (' + jwt_tok[:20] + '...)' if jwt_tok else 'TIDAK ADA'} | "
            f"user={user_hash or 'tidak ada'}"
        )

    except Exception as e:
        logger.warning(f"[WS-JWT] ❌ Exception: {e}")

    return jwt_tok, user_hash


def _scrape_jwt_token(scraper, page_url: str) -> str | None:
    """Wrapper backward-compat — hanya return JWT, abaikan user_hash."""
    tok, _ = _scrape_jwt_and_user(scraper, page_url)
    return tok
...
def _build_livesms_client(email: str, jwt_token: str, user_hash: str = "") -> "_sio_module.Client | None":
    """
    ...
    try:
        ...
        def _parse_live_payload(data: dict, source_label: str) -> dict:
            """
            Parse payload event /livesms sesuai source code iVAS asli.
            iVAS pakai 'originator' bukan 'cli' untuk nomor pengirim.
            """
            # originator = nomor pengirim (dari source iVAS: data.originator.replace("+",""))
            originator = str(data.get("originator", data.get("cli", ""))).replace("+", "")
            # recipient = nomor penerima (nomor virtual iVAS)
            ...
            sid        = str(data.get("sid", ""))
            # statusPaid = (data.client_revenue != 0) ? "Paid" : ...
            revenue    = data.get("client_revenue", data.get("revenue", 0))
            try:
                paid = "Paid" if float(revenue) > 0 else "Unpaid"
            except Exception:
                paid = str(data.get("paid", ""))
            # limit: 1 = dalam limit (bg-success), lainnya = over limit (bg-danger)
            limit_val  = data.get("limit", 1)
            in_limit   = (str(limit_val) == "1")

            return {
                "range":        range_name,
                "phone_number": originator or recipient,   # nomor pengirim
                "originator":   originator,                # field asli iVAS
                "recipient":    recipient,                 # nomor virtual iVAS
                "otp_message":  message,
                "message":      message,
                "sid":          sid,
                "paid":         paid,
        ...
        @sio.on("send_message_live")
        def on_live(data):
            """SMS masuk ke nomor akun sendiri (dalam limit)."""
            try:
                entry = _parse_live_payload(data, "websocket_livesms")
        ...
        @sio.on("send_message_max_Limit_231177")
        def on_limit(data):
            """SMS melebihi limit — tetap simpan dengan flag limit_exceeded."""
            try:
        ...
        def on_any_live(event, data):
            # Log semua event baru — bisa jadi ada event JWT-name di /livesms
            if event not in ("connect", "disconnect", "connect_error",
                             "send_message_live", "send_message_max_Limit_231177"):
                logger.info(f"[WS-LIVE] 🔔 UNKNOWN EVENT '{event}' [{email[:20]}]: {str(data)[:200]}")
                # Tetap coba parse dan simpan — mungkin ini event utama /livesms
                if isinstance(data, dict) and ("message" in data or "originator" in data):
                    try:
                        entry = _parse_live_payload(data, f"websocket_livesms_{event}")
                        _ws_add_live(email, entry)
                        logger.info(f"[WS-LIVE] ✅ Saved unknown event '{event}' sebagai SMS")
                    except Exception:
                        pass
...
def _ws_connect_livesms(account: dict):
    ...
    while _ws_enabled:
        try:
            ...
            scraper   = session["scraper"]
            # Gunakan _scrape_jwt_and_user untuk dapat BOTH token + user hash
            jwt_token, user_hash = _scrape_jwt_and_user(scraper, LIVE_PAGE)
            if not jwt_token:
                jwt_token = scraper.cookies.get("laravel_session", "")

            sio = _build_livesms_client(email, jwt_token or "", user_hash or "")
            if not sio:
                time.sleep(reconnect_delay)
                continue
...
def debug_jwt_user():
    ...
    for acc in targets:
        ...
        try:
            ...
            scraper = session["scraper"]
            jwt_tok, user_hash = _scrape_jwt_and_user(scraper, page_url)
            ...
            out.append(f"  xsrf.startswith('eyJ'): {xsrf.startswith('eyJ') if xsrf else False}")
            out.append(f"  laravel_session : {'✅' if laravel else '❌'} {laravel[:30] + '...' if laravel else 'tidak ada'}")
            out.append(f"  → WS query.token akan: {'XSRF-TOKEN' if xsrf and xsrf.startswith('eyJ') else 'laravel_session' if laravel else '❌ TIDAK ADA'}")
            out.append(f"  → WS query.user  akan: {user_hash or '❌ TIDAK ADA'}")
            out.append("")
...
def debug_raw_livesms():
    ...
    try:
        ...
        if section in ("all", "jwt"):
            ...
            out.append(SEP2)
            jwt_tok, user_hash = _scrape_jwt_and_user(scraper, f"{BASE_URL}/portal/live/my_sms")
