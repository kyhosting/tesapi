...
def _get_number_id(row):
    """
    Ambil number_id dari row.
    Priority: ReturnNumberToSystem(ID) > TerminationDetials(ID) > data-id > field id/number_id
    """
    action = str(row.get("action", "") or "")

    # Priority 1: ReturnNumberToSystem('ID') atau ReturnNumberToSystem(ID)
    m = re.search(r"ReturnNumberToSystem\s*\(\s*['\"]?(\d+)['\"]?\s*\)", action)
    if m:
        return m.group(1)

    # Priority 2: TerminationDetials('ID')
    m = re.search(r"TerminationDetials\s*\(\s*['\"]?(\d+)['\"]?\s*\)", action)
    if m:
        return m.group(1)

    # Priority 3: data-id="ID"
    m = re.search(r'data-id=["\'"]?(\d+)["\'"]?', action)
    if m:
        return m.group(1)

    # Priority 4: angka >= 5 digit pertama di action HTML
    m = re.search(r"(\d{5,})", action)
    if m:
        return m.group(1)

    # Priority 5: field number_id / id / DT_RowId dari row dict
