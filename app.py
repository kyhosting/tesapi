...
def _scrape_jwt_and_user(scraper, page_url: str) -> tuple:
    ...
    try:
        ...
        if all_jwts:
            jwt_tok = all_jwts[0]
            logger.info(f"[WS-JWT] ✅ JWT dari HTML: {jwt_tok[:30]}...")

        # ── Ekstrak user hash — dari source iVAS: query: { user: "hex32chars" } ──
        # Pattern: user:"e76c590db0526e1f8892645bcae9aa5e" atau user: 'xxx'
        user_m = re.search(r"""[,{]\s*user\s*:\s*["']([a-f0-9]{32})["']""", html)
        if user_m:
            user_hash = user_m.group(1)
            logger.info(f"[WS-JWT] ✅ user hash dari HTML: {user_hash}")

        # Cari juga pattern query object: { token: ..., user: ... }
        if not user_hash:
            user_m2 = re.search(
        ...
        # ── Kalau JWT belum ada, coba polling handshake ──
        if not jwt_tok:
            try:
        ...
        # ── Fallback JWT: laravel_session cookie ──
        if not jwt_tok:
        ...
        # ── Fallback user hash: MD5 dari email (beberapa deploy iVAS pakai ini) ──
        if not user_hash:
            import hashlib
            # Ambil email dari cookies atau pakai kosong
            # iVAS pakai MD5(email) lowercase sebagai user hash
            xsrf = scraper.cookies.get("XSRF-TOKEN", "")
            if xsrf and len(xsrf) >= 32:
                user_hash = xsrf[:32].lower()
                logger.info(f"[WS-JWT] user hash fallback XSRF: {user_hash}")

        if jwt_tok:
            logger.info(f"[WS-JWT] Result: token={jwt_tok[:25]}... user={user_hash or 'N/A'}")
        else:
            logger.warning(f"[WS-JWT] ❌ Tidak ada JWT dari {page_url}")
...
def debug_jwt_user():
    """
    Debug endpoint — tampilkan JWT token + user hash yang dipakai iVAS /livesms.

    Dari source iVAS asli:
        window.liveSMSSocket = io('https://ivasms.com:2087/livesms', {
            query: { token: 'eyJ...', user: 'e76c590d...' },
        });

    Query params:
      account=email  → akun tertentu (default: semua)
      page=url       → page untuk scrape JWT (default: /portal/live/my_sms)
    """
    acc_filter = request.args.get("account", "").strip()
    page_url   = request.args.get("page", f"{BASE_URL}/portal/live/my_sms").strip()
    ...
    for acc in targets:
        ...
        try:
            ...
            out.append(f"  JWT token  : {jwt_tok[:50] + '...' if jwt_tok else '❌ tidak ada'}")
            out.append(f"  user hash  : {user_hash or '❌ tidak ada'}")
            out.append(f"  laravel_session: {laravel[:30] + '...' if laravel else '❌'}")
            out.append(f"  XSRF-TOKEN : {xsrf[:30] + '...' if xsrf else '❌'}")
            out.append("")
            ...
            results.append({
                ...
                "user_hash": user_hash,
                "has_laravel_session": bool(laravel),
                "ws_query": {
                    "token": jwt_tok[:30] + "..." if jwt_tok else None,
                    "user":  user_hash,
                }
            })
        except Exception as e:
