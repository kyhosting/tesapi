...
def debug_portal_numbers_html():
    """
    Return raw HTML /portal/numbers — cari JS ReturnNumberToSystem.
    Params: account (opsional), js_only=1 (hanya script tags)
    """
    acc_email = request.args.get("account", "").strip()
    js_only   = request.args.get("js_only", "0") == "1"

    sessions = login_all_accounts()
    if not sessions:
        return "ERROR: Login gagal", 500

    session = next((s for s in sessions if s["email"] == acc_email), sessions[0])
    account = _get_account(session["email"])
    sess    = get_session(account)
    if not sess or not sess.get("ok"):
        return "ERROR: Session gagal", 500

    resp = sess["scraper"].get(
        f"{BASE_URL}/portal/numbers",
        headers={"Accept": "text/html,*/*;q=0.8", "Referer": BASE_URL},
        timeout=20
    )
    html = decode_response(resp)

    if js_only:
        # Hanya return bagian <script> yang mengandung ReturnNumberToSystem
        scripts = re.findall(r"<script[^>]*>(.*?)</script>", html, re.DOTALL | re.IGNORECASE)
        relevant = [s for s in scripts if "ReturnNumberToSystem" in s or "portal/numbers" in s.lower()]
        if not relevant:
            # Return semua script blocks
            relevant = scripts
        header = f"STATUS: {resp.status_code} | LEN: {len(html)} | SCRIPTS: {len(scripts)} relevant: {len(relevant)}"
        body   = header + "\n\n" + "\n\n<!-- NEXT SCRIPT -->\n\n".join(relevant)
