...
def _get_number_id(row):
    """
    Ambil number_id dari row untuk keperluan delete/return.

    DISCOVERY: iVAS menyimpan ID sebenarnya di 2 tempat:
      1. field number_id = HTML checkbox dengan value="REAL_ID"  ← INI yang benar untuk delete
      2. action = ReturnNumberToSystem('LONG_ID')  ← row/DT id, bukan termination id

    Priority: checkbox value di number_id > data-id di action > TerminationDetials > ReturnNumberToSystem
    """
    # Priority 1: checkbox value="ID" di field number_id
    # Format: <input ... value="35800"> — ini ID yang valid untuk /termination/details
    number_id_field = str(row.get("number_id", "") or "")
    m = re.search(r'value=["\']?(\d+)["\']?', number_id_field)
    if m:
        return m.group(1)

    # Priority 2: field number_id langsung berupa angka
    if number_id_field.strip().isdigit():
        return number_id_field.strip()

    action = str(row.get("action", "") or "")

    # Priority 3: data-id="ID" di action HTML
    m = re.search(r'data-id=["\'"]?(\d+)["\'"]?', action)
    if m:
        return m.group(1)

    # Priority 4: TerminationDetials('ID')
    m = re.search(r"TerminationDetials\s*\(\s*['\"]?(\d+)['\"]?\s*\)", action)
    if m:
        return m.group(1)

    # Priority 5: ReturnNumberToSystem('ID') — fallback terakhir
    m = re.search(r"ReturnNumberToSystem\s*\(\s*['\"]?(\d+)['\"]?\s*\)", action)
    if m:
        return m.group(1)

    # Priority 6: field id / DT_RowId
...
def _get_number_id_debug(row):
    """Sama seperti _get_number_id tapi return semua kandidat untuk debug."""
    action = str(row.get("action", "") or "")
    found  = {}

    m = re.search(r"ReturnNumberToSystem\s*\(\s*['\"]?(\d+)['\"]?\s*\)", action)
    if m: found["ReturnNumberToSystem"] = m.group(1)

    m = re.search(r"TerminationDetials\s*\(\s*['\"]?(\d+)['\"]?\s*\)", action)
    if m: found["TerminationDetials"] = m.group(1)

    m = re.search(r'data-id=["\'"]?(\d+)["\'"]?', action)
    if m: found["data-id"] = m.group(1)

    all_nums = re.findall(r"(\d{5,})", action)
    if all_nums: found["all_digits_in_action"] = all_nums

    for key in ("number_id", "id", "DT_RowId"):
        v = str(row.get(key, "")).strip()
        if v: found[f"field_{key}"] = v

    # PENTING: checkbox value di field number_id — ini ID yang valid untuk delete
    number_id_field = str(row.get("number_id", "") or "")
    m_cb = re.search(r'value=["\']?(\d+)["\']?', number_id_field)
    if m_cb: found["checkbox_value_in_number_id"] = m_cb.group(1)

    found["_resolved"] = _get_number_id(row)
    found["action_snippet"] = action[:300]
    return found
...
# ════════════════════════════════════════════════════════
# /debug/my-numbers-raw — debug row data dari My Numbers
# ════════════════════════════════════════════════════════

@app.route("/debug/my-numbers-raw")
def debug_my_numbers_raw():
    """
    Debug raw row data dari My Numbers untuk analisis number_id.
    Tampilkan semua field + kandidat ID dari tiap nomor.

    Params:
      range_name : filter range (opsional)
      limit      : max rows (default 10)
      account    : filter akun (opsional)
    Contoh:
      /debug/my-numbers-raw?range_name=PAKISTAN 34&limit=5
    """
    range_name = request.args.get("range_name", "").strip()
    ...
    target_sessions = [s for s in sessions if s["email"] == acc_email] if acc_email else [sessions[0]]
    session = target_sessions[0]
    email   = session["email"]
    account = _get_account(email)

    search = range_name if range_name else ""
    rows, total = _fetch_my_numbers(account, search=search, length=limit)

    out = []
    SEP = "=" * 70
    out.append(SEP)
    out.append(f"  DEBUG MY NUMBERS RAW — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    out.append(f"  Account    : {email}")
    out.append(f"  Range      : {range_name or '(semua)'}")
    out.append(f"  Found rows : {len(rows)} / {total} total")
    out.append(SEP)

    for i, row in enumerate(rows[:limit]):
        out.append(f"\n--- ROW {i+1} ---")
        # Print semua field (non-action)
        for k, v in row.items():
            if k != "action":
                out.append(f"  {k:20s} = {str(v)[:120]}")
        # Analisis action khusus
        action = str(row.get("action", "") or "")
        out.append(f"  {'action_len':20s} = {len(action)} chars")
        out.append(f"  {'action_raw':20s} = {action[:300]}")
        # Debug ID candidates
        debug = _get_number_id_debug(row)
        out.append(f"  --- ID CANDIDATES ---")
        for k, v in debug.items():
            out.append(f"    {k:30s} = {str(v)[:120]}")
...
# ════════════════════════════════════════════════════════
# /debug/return-number — test semua kemungkinan endpoint return
# ════════════════════════════════════════════════════════

@app.route("/debug/return-number")
def debug_return_number():
    """
    Test semua kemungkinan endpoint untuk ReturnNumberToSystem.
    Pakai number_id dari ReturnNumberToSystem() di action HTML.

    Params:
      number_id : ID dari ReturnNumberToSystem (dari /debug/my-numbers-raw)
      account   : (opsional) filter akun
    Contoh:
      /debug/return-number?number_id=3580054752
    """
    number_id = request.args.get("number_id", "").strip()
    acc_email  = request.args.get("account", "").strip()
    ...
    if not number_id:
        return (
            "ERROR: ?number_id= wajib\n"
            "Ambil number_id dari /debug/my-numbers-raw field ReturnNumberToSystem\n"
            "Contoh: /debug/return-number?number_id=3580054752\n"
        ), 400
    ...
    SEP = "=" * 70
    out.append(SEP)
    out.append(f"  DEBUG RETURN NUMBER — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    out.append(f"  Account   : {email}")
    out.append(f"  number_id : {number_id}")
    out.append(SEP)
    ...
    # Semua kandidat endpoint + payload yang mungkin dipakai ReturnNumberToSystem
    candidates = [
        ("POST", f"{BASE_URL}/portal/numbers/termination/details",         {"id": number_id}),
        ("POST", f"{BASE_URL}/portal/numbers/return",                       {"id": number_id}),
        ("POST", f"{BASE_URL}/portal/numbers/return",                       {"number_id": number_id}),
        ("POST", f"{BASE_URL}/portal/numbers/my/return",                    {"id": number_id}),
        ("POST", f"{BASE_URL}/portal/number/return",                        {"id": number_id}),
        ("POST", f"{BASE_URL}/portal/numbers/my/delete",                    {"id": number_id}),
        ("POST", f"{BASE_URL}/portal/number/my/delete",                     {"id": number_id}),
        ("POST", f"{BASE_URL}/portal/numbers/delete",                       {"id": number_id}),
        ("POST", f"{BASE_URL}/portal/numbers/termination/return",           {"id": number_id}),
        ("POST", f"{BASE_URL}/portal/numbers/termination/number/return",    {"id": number_id}),
        ("POST", f"{BASE_URL}/portal/numbers/termination/number/delete",    {"id": number_id}),
        ("GET",  f"{BASE_URL}/portal/numbers/return/{number_id}",           {}),
        ("GET",  f"{BASE_URL}/portal/numbers/my/return/{number_id}",        {}),
        ("POST", f"{BASE_URL}/portal/numbers/termination/details",         {"number_id": number_id}),
        ("POST", f"{BASE_URL}/portal/numbers/termination/details",         {"id": number_id, "type": "return"}),
    ]

    for method, url, data in candidates:
        try:
            resp, _ = do_request(account, method, url, data=data, headers=hdrs)
            if resp is None:
                out.append(f"\n[NULL ] {method} {url.replace(BASE_URL,'')} data={data}")
                continue
            body = decode_response(resp)
            redirect = "/login" in getattr(resp, "url", "")
            preview  = body[:200].replace("\n", " ").strip()
            label = "✓ OK  " if resp.status_code in (200, 201) else f"✗ {resp.status_code}"
            redir_note = " ← REDIRECT LOGIN" if redirect else ""
            out.append(f"\n[{label}] {method} {url.replace(BASE_URL,'')}")
            out.append(f"  data    : {data}")
            out.append(f"  status  : {resp.status_code}{redir_note}")
            out.append(f"  ct      : {resp.headers.get('Content-Type','?')[:60]}")
            out.append(f"  preview : {preview[:300]}")
        except Exception as e:
            out.append(f"\n[ERROR] {method} {url.replace(BASE_URL,'')} — {e}")
...
# ════════════════════════════════════════════════════════
# /debug/bulk-return-test — test bulk return via select_id[]
# ════════════════════════════════════════════════════════

@app.route("/debug/bulk-return-test")
def debug_bulk_return_test():
    """
    Test bulk return menggunakan select_id[] (checkbox pattern dari iVAS).
    
    Dari debug: checkbox name="select_id[]" value="35800"
    → iVAS kemungkinan pakai bulk action dengan array select_id[]
    
    Params:
      ids      : comma-separated IDs, misal "35800,35801,35802"
      account  : (opsional)
    Contoh:
      /debug/bulk-return-test?ids=35800
    """
    ...
    if not ids_raw:
        return (
            "ERROR: ?ids= wajib\n"
            "Contoh: /debug/bulk-return-test?ids=35800\n"
            "IDs dari checkbox value di /debug/my-numbers-raw\n"
        ), 400
    ...
    SEP = "=" * 70
    out.append(SEP)
    out.append(f"  DEBUG BULK RETURN TEST — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    out.append(f"  Account : {email}")
    ...
    # Build select_id[] payload
    # requests encode list as multiple values: select_id[]=35800&select_id[]=35801
    select_payload = [("select_id[]", i) for i in ids]

    candidates = [
        # Bulk via select_id[]
        ("bulk_select_id[]→termination/bulk",
         f"{BASE_URL}/portal/numbers/termination/bulk",
         select_payload),
        
        ("bulk_select_id[]→numbers/bulk-return",
         f"{BASE_URL}/portal/numbers/bulk-return",
         select_payload),
        ...
        ("bulk_select_id[]→numbers/return-selected",
         f"{BASE_URL}/portal/numbers/return-selected",
         select_payload),

        ("bulk_select_id[]→numbers/termination/bulk-return",
         f"{BASE_URL}/portal/numbers/termination/bulk-return",
         select_payload),

        # Dengan action field tambahan
        ("bulk_select_id[]+action→termination/bulk",
         f"{BASE_URL}/portal/numbers/termination/bulk",
         select_payload + [("action", "return")]),

        ("bulk_select_id[]+action→numbers/action",
         f"{BASE_URL}/portal/numbers/action",
         select_payload + [("action", "return")]),

        # Single id tapi pakai key select_id
        ("select_id→termination/details",
         f"{BASE_URL}/portal/numbers/termination/details",
         [("select_id", ids[0])]),
    ...
    for label, url, payload in candidates:
        try:
            session_obj = get_session(account)
            if not session_obj or not session_obj.get("ok"):
                out.append(f"\n[ERROR] Login gagal")
                continue
            scraper = session_obj["scraper"]
            csrf    = session_obj["csrf"]
            
            # Build final payload dengan _token
            final_payload = list(payload) + [("_token", csrf)]
            
            resp = scraper.post(url, data=final_payload, headers=hdrs)
            body = decode_response(resp)
            redirect = "/login" in getattr(resp, "url", "")
            preview  = body[:200].replace("\n", " ").strip()
            label_ok = "✓ OK  " if resp.status_code in (200, 201) else f"✗ {resp.status_code}"
            redir_note = " ← REDIRECT LOGIN" if redirect else ""
            out.append(f"\n[{label_ok}] {label}")
            out.append(f"  url     : {url.replace(BASE_URL,'')}")
            out.append(f"  payload : {[(k,v) for k,v in final_payload if k != '_token']}")
            out.append(f"  status  : {resp.status_code}{redir_note}")
            out.append(f"  preview : {preview[:300]}")
        except Exception as e:
            out.append(f"\n[ERROR] {label} — {e}")
...
# ════════════════════════════════════════════════════════
# /debug/portal-numbers-html — raw HTML + JS dari /portal/numbers
# ════════════════════════════════════════════════════════
...
@app.route("/debug/portal-numbers-html")
def debug_portal_numbers_html():
    """
    Raw HTML halaman /portal/numbers untuk analisis JS ReturnNumberToSystem.
    Params: full=1 (full HTML), account (opsional)
    Contoh: /debug/portal-numbers-html
    """
    acc_email = request.args.get("account", "").strip()
    full      = request.args.get("full", "0").strip() == "1"
    ...
    out.append(f"\nTotal script blocks: {len(script_blocks)}")

    out += ["\n" + DASH, "FUNGSI ReturnNumberToSystem:", DASH]
    found_func = False
    for i, blk in enumerate(script_blocks):
        if "ReturnNumberToSystem" in blk:
            idx   = blk.find("ReturnNumberToSystem")
            chunk = blk[max(0, idx - 300):idx + 2000]
            out.append(f"\n[Script block {i+1}]")
            out.append(chunk)
            found_func = True
    if not found_func:
        idx = html.find("ReturnNumberToSystem")
        if idx >= 0:
            out.append("Ditemukan di raw HTML:")
            out.append(html[max(0, idx - 300):idx + 2000])
        else:
            out.append("TIDAK DITEMUKAN sama sekali")

    out += ["\n" + DASH, "AJAX URL CANDIDATES:", DASH]
    for i, blk in enumerate(script_blocks):
        urls = re.findall(r"url\s*:\s*'([^']+)'", blk)
        urls += re.findall(r'url\s*:\s*"([^"]+)"', blk)
        urls += re.findall(r"url\s*=\s*'([^']+)'", blk)
        urls += re.findall(r'url\s*=\s*"([^"]+)"', blk)
        if urls:
    ...
    out += ["\n" + DASH, "FORM ACTIONS:", DASH]
    form_pats = re.findall(r"action='([^']+)'", html) + re.findall(r'action="([^"]+)"', html)
    for fa in form_pats:
        out.append("  " + fa)

    if full:
        out += ["\n" + DASH, "FULL HTML:", DASH, html]
    else:
        out.append("\n(Tambahkan ?full=1 untuk lihat full HTML)")
...
def delete_numbers_by_range():
    """
    Delete/return SEMUA nomor dalam satu range ke sistem.

    Flow:
      1. Fetch My Numbers → filter by range_name
      2. POST /portal/numbers/termination/details { id: number_id } untuk tiap nomor
      3. Return ringkasan hasil

    Params:
      range_name : nama range yang mau di-delete semua nomornya, misal "PAKISTAN 34"
      account    : (opsional) filter 1 akun, default: semua akun
    Contoh:
    ...
    if not range_name:
        return jsonify({
            ...
            "tip":     "Cek /numbers/my-list untuk lihat range_name yang tersedia",
        }), 400
    ...
    # ── Step 1: Kumpulkan semua number_id dalam range ini ───────────────────
    to_delete = []  # list of {"email": ..., "number_id": ..., "number": ...}
    lock = threading.Lock()

    def _find_numbers(session):
        email   = session["email"]
        account = _get_account(email)
        if not account:
            return
        # Cari di My Numbers dengan search = range_name
        rows, _ = _fetch_my_numbers(account, search=range_name, length=500)
        for row in rows:
            row_range = re.sub(r"<[^>]+>","",str(row.get("range",""))).strip()
            # Match exact range name (case-insensitive)
        ...
        logger.info(f"[DEL-RANGE] {email}: ditemukan {len([x for x in to_delete if x['email']==email])} nomor di range '{range_name}'")

    with ThreadPoolExecutor(max_workers=max(len(targets), 1)) as ex:
        list(ex.map(_find_numbers, targets))

    if not to_delete:
        return jsonify({
            "status":     "error",
            "error":      f"Tidak ada nomor ditemukan di range '{range_name}'",
            "range_name": range_name,
            "tip":        "Cek /numbers/my-list untuk lihat range yang tersedia",
        }), 404

    # ── Step 2: Delete semua nomor yang ditemukan ────────────────────────────
    results, errors = [], []
    ...
    def _delete_one(item):
        ...
        try:
            ...
            # Coba beberapa endpoint — ReturnNumberToSystem bisa map ke endpoint berbeda
            success, message, resp = False, "", None
            for _url, _data in [
                (f"{BASE_URL}/portal/numbers/termination/details",      {"id": nid}),
                (f"{BASE_URL}/portal/numbers/return",                   {"id": nid}),
                (f"{BASE_URL}/portal/numbers/my/return",                {"id": nid}),
                (f"{BASE_URL}/portal/number/my/delete",                 {"id": nid}),
                (f"{BASE_URL}/portal/numbers/my/delete",                {"id": nid}),
                (f"{BASE_URL}/portal/numbers/termination/number/return",{"id": nid}),
            ]:
                resp, _ = do_request(account, "POST", _url, data=_data, headers=hdrs_del)
                success, message, _ = _parse_ivas_resp(resp)
                if success:
                    logger.info(f"[DEL-RANGE] {email}: nid={nid} ✓ via {_url}")
                    break
                # Kalau "No query results" berarti endpoint salah, coba berikutnya
                if "no query" in message.lower() or "not found" in message.lower() or (resp and resp.status_code == 404):
                    continue
                # Kalau error lain (misal 403/500) mungkin endpoint benar tapi ada masalah lain
                break
            ...
            with lock:
                (results if success else errors).append(entry)
            logger.info(f"[DEL-RANGE] {email}: nid={nid} num={num} success={success} msg={message}")
        except Exception as e:
            with lock:
                errors.append({
                    "account":   email,
    ...
    with ThreadPoolExecutor(max_workers=max(len(to_delete), 1)) as ex:
        list(ex.map(_delete_one, to_delete))
